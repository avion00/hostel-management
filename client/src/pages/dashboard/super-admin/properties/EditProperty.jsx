...
const EditProperty = () => {
  ...
  const fetchPropertyDetails = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        const hostel = response.data.data.hostel;
        setFormData({
          ...
          near_college: hostel.near_college || "",
          total_rooms: hostel.total_rooms || "",
          price_starting: hostel.price_starting || "",
          amenities: hostel.amenities || [],
        });

        if (hostel.images && Array.isArray(hostel.images)) {
          setImagePreviews(hostel.images.map((img) => `http://localhost:5000${img}`));
        }
      } else {
        toast.error(response.data.message || "Failed to load property details");
      }
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Update property with multipart/form-data
      // Don't set Content-Type header - browser will set it automatically with boundary
      const response = await axiosInstance.patch(`/admin/hostels/${id}`, formDataToSend);
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-slate-600">
              Update details for this hostel property
            </p>
          </div>
