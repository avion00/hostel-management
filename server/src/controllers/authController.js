...
export const login = async (req, res) => {
  try {
    ...
    try {
      ...
      console.log('Lock check skipped:', lockError.message);
    }
    ...
    if (!isPasswordValid) {
      ...
      try {
        ...
        console.log('Login attempt recording skipped');
      }
      ...
      try {
        ...
        if (attemptResult && attemptResult.locked) {
          const minutesLocked = 15; // 15 minutes
          return res.status(423).json({
            success: false,
        ...
        if (attemptResult && attemptResult.remaining) {
          return res.status(401).json({
            success: false,
            message: 'Invalid email or password',
        ...
        console.log('Failed attempt increment skipped');
      }
    ...
    try {
      ...
      console.log('Reset failed attempts skipped');
    }
    ...
    // Remove password and integer ID from response
    delete user.password;
    const userId = user.id; // Keep for token generation
    delete user.id; // Remove integer ID from response
